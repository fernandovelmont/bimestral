...
public class ControladorDireccion {
    ...
    @Autowired
    RepositorioDireccion repo;
//Primero hacemos insertar
     ...
     * @return El Tipo de retorno es el objeto Direccion que se guardo e incluye
     * el id generado
     */
    ...
    @RequestMapping(value = "/direccion/{id}/{numero}/{calle}/{cp}/{municipio}",
            method = RequestMethod.POST, headers = {"Accept=application/json"})
    public Direccion guardar(@PathVariable Long id, @PathVariable Integer numero, @PathVariable String calle, @PathVariable Long cp, @PathVariable String municipio) {
    ...
    @RequestMapping(value="/direccion/{id}",method=RequestMethod.GET,headers={"Accept=application/json"})
    public Direccion buscarPorId(@PathVariable Long id){
        return repo.findOne(id);
    }
    
    @RequestMapping(value="/direccion",method=RequestMethod.GET,headers={"Accept=application/json"})
